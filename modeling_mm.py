...
from projector import MeanPoolingProjector, TemporalPoolingProjector
...
class MMForCausalLM(PreTrainedModel):
    ...
    def get_vision_model(self, config, load_checkpoint):
        ...
        kernel = self.config.pooling_kernel
        ...
        elif "mean" in str(self.config.mm_projector_type).lower():
            if "after" in str(self.config.mm_projector_type).lower():
                self.mm_projector = MeanPoolingProjector(input_dim=self.config.visual_hidden_dim, output_dim=config.hidden_size, kernel=kernel, pool_after=True)
            else:
                self.mm_projector = MeanPoolingProjector(input_dim=self.config.visual_hidden_dim, output_dim=config.hidden_size, kernel=kernel)
            
            self.feature_map_size = int(self.config.pesudo_img_size / (self.config.pesudo_patch_size))
            self.num_queries = math.floor((self.config.pesudo_img_size / (self.config.pesudo_patch_size * kernel)))**2
        elif "temp" in str(self.config.mm_projector_type).lower():
            if "after" in str(self.config.mm_projector_type).lower():
                self.mm_projector = TemporalPoolingProjector(input_dim=self.config.visual_hidden_dim, output_dim=config.hidden_size, spatial_kernel=kernel, pool_after=True)
            else:
                self.mm_projector = TemporalPoolingProjector(input_dim=self.config.visual_hidden_dim, output_dim=config.hidden_size, spatial_kernel=kernel)
            self.feature_map_size = int(self.config.pesudo_img_size / (self.config.pesudo_patch_size))
            self.num_queries = math.floor((self.config.pesudo_img_size / (self.config.pesudo_patch_size * kernel)))**2
